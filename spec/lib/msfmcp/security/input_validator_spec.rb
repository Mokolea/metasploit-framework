...
RSpec.describe MsfMcp::Security::InputValidator do
  describe '.validate_parameter!' do
    context 'with Array constraint (enum)' do
      ...
      it 'rejects value not in the list' do
        ...
        }.to raise_error(MsfMcp::Security::ValidationError, 'Invalid color: "yellow". Must be one of: red, green, blue')
      end

      it 'includes the parameter name in error' do
        expect {
          described_class.validate_parameter!('fruit', 'pear', %w[apple banana])
        }.to raise_error(MsfMcp::Security::ValidationError, 'Invalid fruit: "pear". Must be one of: apple, banana')
      end
    end
    ...
    context 'with Range constraint' do
      ...
      it 'rejects value outside range' do
        ...
        }.to raise_error(MsfMcp::Security::ValidationError, 'port must be between 1 and 65535: 0')
      end

      it 'rejects non-integer value' do
        expect {
          described_class.validate_parameter!('port', 'abc', 1..65535)
        }.to raise_error(MsfMcp::Security::ValidationError, 'port must be an integer: "abc"')
      end

      it 'rejects nil value' do
        expect {
          described_class.validate_parameter!('port', nil, 1..65535)
        }.to raise_error(MsfMcp::Security::ValidationError, 'port cannot be nil')
      end
      ...
      context 'with Range value (range-in-range)' do
        ...
        it 'rejects range starting below constraint' do
          ...
          }.to raise_error(MsfMcp::Security::ValidationError, 'ports must be between 1 and 65535: 0..443')
        end

        it 'rejects range ending above constraint' do
          expect {
            described_class.validate_parameter!('ports', 80..70000, 1..65535)
          }.to raise_error(MsfMcp::Security::ValidationError, 'ports must be between 1 and 65535: 80..70000')
        end

        it 'rejects backwards range' do
          expect {
            described_class.validate_parameter!('ports', 443..80, 1..65535)
          }.to raise_error(MsfMcp::Security::ValidationError, 'ports must be between 1 and 65535: 443..80')
        end

        it 'rejects range with non-integer bounds' do
          expect {
            described_class.validate_parameter!('ports', 'a'..'z', 1..65535)
          }.to raise_error(MsfMcp::Security::ValidationError, 'ports must have integer bounds: "a".."z"')
        end
      end
    end
    ...
    context 'with Regexp constraint' do
      ...
      it 'rejects non-matching value' do
        ...
        }.to raise_error(MsfMcp::Security::ValidationError, 'Invalid name format: has spaces')
      end
    end
    ...
    context 'with allow_nil option' do
      ...
      it 'rejects nil when allow_nil is false' do
        ...
        }.to raise_error(MsfMcp::Security::ValidationError, 'proto cannot be nil')
      end
      ...
      it 'still validates non-nil values when allow_nil is true' do
        ...
        }.to raise_error(MsfMcp::Security::ValidationError, 'Invalid proto: "icmp". Must be one of: tcp, udp')
      end
    end
    ...
    context 'with invalid IP addresses' do
      it 'rejects malformed IPv4' do
        ['256.1.1.1', '192.168.1', '192.168.1.1.1', 'a.b.c.d'].each do |addr|
          ...
          }.to raise_error(MsfMcp::Security::ValidationError, "Invalid IP address or CIDR: #{addr}")
        end
      end
      ...
      it 'rejects out of range octets' do
        ...
        }.to raise_error(MsfMcp::Security::ValidationError, 'Invalid IP address or CIDR: 192.168.300.1')
      end
  ...
  describe '.validate_port_range!' do
    ...
    context 'with invalid ports' do
      ...
      it 'rejects port above 65535' do
        ...
        }.to raise_error(MsfMcp::Security::ValidationError, 'Port must be between 1 and 65535: 65536')
      end

      it 'rejects negative ports' do
        expect {
          described_class.validate_port_range!(-1)
        }.to raise_error(MsfMcp::Security::ValidationError, 'Port must be between 1 and 65535: -1')
      end
    end
    ...
    context 'with invalid range formats' do
      it 'rejects backwards range' do
        ...
        }.to raise_error(MsfMcp::Security::ValidationError, 'Port range must be between 1 and 65535: 100..50')
      end

      it 'rejects "1-"' do
        expect {
          described_class.validate_port_range!('1-')
        }.to raise_error(MsfMcp::Security::ValidationError, 'Port must be an integer: "1-"')
      end

      it 'rejects "-100"' do
        expect {
          described_class.validate_port_range!('-100')
        }.to raise_error(MsfMcp::Security::ValidationError, 'Port must be between 1 and 65535: -100')
      end

      it 'rejects "abc-def"' do
        expect {
          described_class.validate_port_range!('abc-def')
        }.to raise_error(MsfMcp::Security::ValidationError, 'Port range must have integer bounds: abc-def')
      end
    end
  ...
  describe '.validate_only_up!' do
    ...
    context 'with invalid values' do
      it 'rejects string "true"' do
        ...
        }.to raise_error(MsfMcp::Security::ValidationError, 'Invalid only_up: "true". Must be one of: true, false')
      end

      it 'rejects string "false"' do
        expect {
          described_class.validate_only_up!('false')
        }.to raise_error(MsfMcp::Security::ValidationError, 'Invalid only_up: "false". Must be one of: true, false')
      end

      it 'rejects nil' do
        expect {
          described_class.validate_only_up!(nil)
        }.to raise_error(MsfMcp::Security::ValidationError, 'only_up cannot be nil')
      end

      it 'rejects integer 1' do
        expect {
          described_class.validate_only_up!(1)
        }.to raise_error(MsfMcp::Security::ValidationError, 'Invalid only_up: 1. Must be one of: true, false')
      end

      it 'rejects integer 0' do
        expect {
          described_class.validate_only_up!(0)
        }.to raise_error(MsfMcp::Security::ValidationError, 'Invalid only_up: 0. Must be one of: true, false')
      end
    end
  end
  ...
  describe '.validate_protocol!' do
    ...
    context 'with invalid protocols' do
      it 'rejects icmp' do
        ...
        }.to raise_error(MsfMcp::Security::ValidationError, 'Invalid Protocol: "icmp". Must be one of: tcp, udp')
      end

      it 'rejects http' do
        expect {
          described_class.validate_protocol!('http')
        }.to raise_error(MsfMcp::Security::ValidationError, 'Invalid Protocol: "http". Must be one of: tcp, udp')
      end

      it 'rejects random string' do
        expect {
          described_class.validate_protocol!('invalid')
        }.to raise_error(MsfMcp::Security::ValidationError, 'Invalid Protocol: "invalid". Must be one of: tcp, udp')
      end
    end
  end
  ...
  describe '.validate_search_query!' do
    ...
    context 'with invalid search queries' do
      it 'rejects empty string' do
        ...
        }.to raise_error(MsfMcp::Security::ValidationError, 'Search query cannot be empty')
      end

      it 'rejects nil' do
        expect {
          described_class.validate_search_query!(nil)
        }.to raise_error(MsfMcp::Security::ValidationError, 'Search query cannot be nil')
      end
      ...
      it 'rejects very long queries' do
        ...
        }.to raise_error(MsfMcp::Security::ValidationError, 'Search query too long (max 500 characters)')
      end

      it 'rejects non printable characters' do
        expect {
          described_class.validate_search_query!("bad\x15\x10")
        }.to raise_error(MsfMcp::Security::ValidationError, /^Invalid Search query format:/)
      end
    end
  end
  ...
  describe '.validate_limit!' do
    ...
    it 'rejects zero' do
      ...
      }.to raise_error(MsfMcp::Security::ValidationError, 'Limit must be between 1 and 1000: 0')
    end

    it 'rejects negative number' do
      expect {
        described_class.validate_limit!(-10)
      }.to raise_error(MsfMcp::Security::ValidationError, 'Limit must be between 1 and 1000: -10')
    end

    it 'rejects excessive limit' do
      expect {
        described_class.validate_limit!(10001)
      }.to raise_error(MsfMcp::Security::ValidationError, 'Limit must be between 1 and 1000: 10001')
    end
  ...
  describe '.validate_offset!' do
    ...
    it 'rejects negative offset' do
      ...
      }.to raise_error(MsfMcp::Security::ValidationError, 'Offset must be between 0 and 1000: -10')
    end

    it 'rejects non-integer offset' do
      expect {
        described_class.validate_offset!('abc')
      }.to raise_error(MsfMcp::Security::ValidationError, 'Offset must be an integer: "abc"')
    end

    it 'rejects excessive offset' do
      expect {
        described_class.validate_offset!(10001)
      }.to raise_error(MsfMcp::Security::ValidationError, 'Offset must be between 0 and 1000: 10001')
    end
  end
  ...
  describe '.validate_pagination!' do
    ...
    context 'with invalid pagination parameters' do
      ...
      it 'rejects invalid offset' do
        ...
        }.to raise_error(MsfMcp::Security::ValidationError, 'Offset must be between 0 and 1000: -5')
      end
  ...
  describe '.validate_module_type!' do
    ...
    context 'with invalid module types' do
      it 'rejects invalid type' do
        ...
        }.to raise_error(MsfMcp::Security::ValidationError, 'Invalid Module type: "invalid". Must be one of: exploit, auxiliary, post, payload, encoder, evasion, nop')
      end

      it 'rejects uppercase type' do
        expect {
          described_class.validate_module_type!('EXPLOIT')
        }.to raise_error(MsfMcp::Security::ValidationError, 'Invalid Module type: "EXPLOIT". Must be one of: exploit, auxiliary, post, payload, encoder, evasion, nop')
      end

      it 'rejects empty string' do
        expect {
          described_class.validate_module_type!('')
        }.to raise_error(MsfMcp::Security::ValidationError, 'Module type cannot be empty')
      end

      it 'rejects nil' do
        expect {
          described_class.validate_module_type!(nil)
        }.to raise_error(MsfMcp::Security::ValidationError, 'Module type cannot be nil')
      end

      it 'rejects scanner (not a valid type)' do
        expect {
          described_class.validate_module_type!('scanner')
        }.to raise_error(MsfMcp::Security::ValidationError, 'Invalid Module type: "scanner". Must be one of: exploit, auxiliary, post, payload, encoder, evasion, nop')
      end
    end
  end
  ...
  describe '.validate_module_name!' do
    ...
    context 'with invalid module names' do
      it 'rejects empty string' do
        ...
        }.to raise_error(MsfMcp::Security::ValidationError, 'Module name cannot be empty')
      end

      it 'rejects nil' do
        expect {
          described_class.validate_module_name!(nil)
        }.to raise_error(MsfMcp::Security::ValidationError, 'Module name cannot be nil')
      end

      it 'rejects whitespace-only string' do
        expect {
          described_class.validate_module_name!('   ')
        }.to raise_error(MsfMcp::Security::ValidationError, 'Invalid Module name format:    ')
      end
      ...
      it 'rejects very long module names' do
        ...
        }.to raise_error(MsfMcp::Security::ValidationError, 'Module name too long (max 500 characters)')
      end

      it 'rejects module name with spaces' do
        expect {
          described_class.validate_module_name!('exploit/windows/my exploit')
        }.to raise_error(MsfMcp::Security::ValidationError, 'Invalid Module name format: exploit/windows/my exploit')
      end

      it 'rejects module name with special characters' do
        expect {
          described_class.validate_module_name!('exploit/windows/test@exploit')
        }.to raise_error(MsfMcp::Security::ValidationError, 'Invalid Module name format: exploit/windows/test@exploit')
      end

      it 'rejects module name with dots' do
        expect {
          described_class.validate_module_name!('exploit/windows/../etc/passwd')
        }.to raise_error(MsfMcp::Security::ValidationError, 'Invalid Module name format: exploit/windows/../etc/passwd')
      end

      it 'rejects module name with backslashes' do
        expect {
          described_class.validate_module_name!('exploit\\windows\\test')
        }.to raise_error(MsfMcp::Security::ValidationError, 'Invalid Module name format: exploit\windows\test')
      end
    end
  end
  ...
  describe 'fuzzing tests' do
    it 'handles random IP-like strings' do
      1000.times do
        ...
        rescue MsfMcp::Security::ValidationError
