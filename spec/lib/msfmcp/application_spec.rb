...
RSpec.describe MsfMcp::Application do
  ...
  describe '#initialize_logger' do
    ...
    it 'creates logger with enabled state when --enable-logging is set' do
      ...
      expect(app.logger).to be_a(MsfMcp::Logging::Logger)
    end
  ...
  describe '#load_configuration' do
    ...
    it 'raises error for missing file' do
      ...
      expect { app.send(:load_configuration) }.to raise_error(MsfMcp::Config::ConfigurationError, /not found/)
    end
  end
  ...
  describe '#validate_configuration' do
    ...
    it 'raises error for invalid configuration' do
      ...
      expect { app.send(:validate_configuration) }.to raise_error(MsfMcp::Config::ValidationError)
    end
  end
  ...
  describe '#initialize_metasploit_client' do
    it 'creates Metasploit client with config values' do
      ...
      mock_client = instance_double(MsfMcp::Metasploit::Client)
      expect(MsfMcp::Metasploit::Client).to receive(:new).with(
  ...
  describe '#authenticate_metasploit' do
    let(:mock_client) { instance_double(MsfMcp::Metasploit::Client) }
  ...
  describe '#initialize_mcp_server' do
    it 'creates MCP server with dependencies' do
      mock_client = instance_double(MsfMcp::Metasploit::Client)
      mock_rate_limiter = instance_double(MsfMcp::Security::RateLimiter)
      mock_logger = instance_double(MsfMcp::Logging::Logger)
      mock_mcp_server = instance_double(MsfMcp::MCP::Server)
      ...
      expect(MsfMcp::MCP::Server).to receive(:new).with(
  ...
  describe '#start_mcp_server' do
    let(:mock_mcp_server) { instance_double(MsfMcp::MCP::Server) }
  ...
  describe '#ensure_rpc_server' do
    context 'when RPC is already available' do
      it 'does not create an RPC manager' do
        ...
        mock_rpc_manager = instance_double(MsfMcp::RpcManager)
        allow(MsfMcp::RpcManager).to receive(:new).and_return(mock_rpc_manager)
    ...
    context 'when --no-auto-start-rpc is set' do
      it 'sets auto_start_rpc to false in config during load_configuration' do
        ...
        expect(MsfMcp::RpcManager).to receive(:new).with(
    ...
    context 'when RPC startup fails' do
      it 'propagates RpcStartupError' do
        ...
        allow(mock_rpc_manager).to receive(:ensure_rpc_available).and_raise(
          MsfMcp::Metasploit::RpcStartupError.new('msfrpcd not found')
        )

        expect { app.send(:ensure_rpc_server) }.to raise_error(MsfMcp::Metasploit::RpcStartupError)
      end
    end
    ...
    context 'when connection times out' do
      it 'propagates ConnectionError' do
        ...
        allow(MsfMcp::RpcManager).to receive(:new).and_return(mock_rpc_manager)
        allow(mock_rpc_manager).to receive(:ensure_rpc_available).and_raise(
          MsfMcp::Metasploit::ConnectionError.new('Timed out waiting for RPC server')
        )

        expect { app.send(:ensure_rpc_server) }.to raise_error(MsfMcp::Metasploit::ConnectionError)
      end
    end
  ...
  describe 'error handlers' do
    describe '#handle_configuration_error' do
      it 'outputs error message and exits' do
        app = described_class.new([], output: output)
        error = MsfMcp::Config::ValidationError.new({ 'msf_api.type': 'is invalid' })
    ...
    describe '#handle_connection_error' do
      it 'outputs connection error with host and port' do
        app = described_class.new([], output: output)
        app.instance_variable_set(:@config, valid_config)
        error = MsfMcp::Metasploit::ConnectionError.new('Connection refused')
    ...
    describe '#handle_api_error' do
      it 'outputs API error message' do
        app = described_class.new([], output: output)
        error = MsfMcp::Metasploit::APIError.new('Invalid method')
    ...
    describe '#handle_authentication_error' do
      it 'outputs authentication error with username' do
        app = described_class.new([], output: output)
        app.instance_variable_set(:@config, valid_config)
        error = MsfMcp::Metasploit::AuthenticationError.new('Login Failed')
  ...
  describe '#run' do
    ...
    it 'runs through the complete startup sequence successfully' do
      ...
      allow(MsfMcp::Metasploit::Client).to receive(:new).and_return(mock_client)
      allow(mock_client).to receive(:authenticate)
      allow(MsfMcp::MCP::Server).to receive(:new).and_return(mock_mcp_server)
    ...
    it 'handles authentication errors gracefully' do
      ...
      allow(mock_client).to receive(:authenticate).and_raise(
        MsfMcp::Metasploit::AuthenticationError.new('Login Failed')
      )
      allow(Signal).to receive(:trap)
    ...
    it 'handles connection errors gracefully' do
      ...
      allow(MsfMcp::Metasploit::Client).to receive(:new).and_return(mock_client)
      allow(mock_client).to receive(:authenticate).and_raise(
        MsfMcp::Metasploit::ConnectionError.new('Connection refused')
      )
      allow(Signal).to receive(:trap)
    ...
    it 'includes ensure_rpc_server in the run sequence before initialize_metasploit_client' do
      ...
      allow(MsfMcp::Metasploit::Client).to receive(:new) { order << :init_client; mock_client }
