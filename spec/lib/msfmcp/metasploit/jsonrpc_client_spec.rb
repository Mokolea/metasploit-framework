...
RSpec.describe MsfMcp::Metasploit::JsonRpcClient do
  let(:host) { 'localhost' }
  let(:port) { 8081 }
  ...
  describe '#call_api' do
    ...
    it 'raises APIError when response contains error' do
      ...
      }.to raise_error(MsfMcp::Metasploit::APIError, 'Invalid method')
    end
    ...
    it 'raises APIError with default message when error has no message' do
      ...
      expect {
        client.call_api('invalid.method')
      }.to raise_error(MsfMcp::Metasploit::APIError, 'Unknown error')
    end
  end
