...
RSpec.describe MsfMcp::Metasploit::Client do
  let(:jsonrpc_client) { double('MsfMcp::Metasploit::JsonRpcClient') }
  let(:messagepack_client) { double('MsfMcp::Metasploit::MessagePackClient') }

  describe '#initialize with JSON-RPC' do
    it 'creates JsonRpcClient for json-rpc api_type' do
      expect(MsfMcp::Metasploit::JsonRpcClient).to receive(:new).with(
        host: 'localhost',
        port: 8081,
        ssl: true,
  ...
  describe '#initialize with MessagePack' do
    it 'creates MessagePackClient for messagepack api_type' do
      expect(MsfMcp::Metasploit::MessagePackClient).to receive(:new).with(
        host: 'localhost',
        port: 55553,
        ssl: true,
        endpoint: '/api/',
  ...
  describe '#initialize with invalid API type' do
    it 'raises Error for unknown api_type' do
      ...
      }.to raise_error(MsfMcp::Error, /Invalid API type/)
    end
  end
  ...
  describe '#create_client' do
    it 'uses default MessagePack endpoint when none provided' do
      expect(MsfMcp::Metasploit::MessagePackClient).to receive(:new).with(
        host: 'localhost',
        port: 55553,
        endpoint: MsfMcp::Metasploit::MessagePackClient::DEFAULT_ENDPOINT,
        ssl: true,
        logger: nil
      )
    ...
    it 'uses default JSON-RPC endpoint when none provided' do
      expect(MsfMcp::Metasploit::JsonRpcClient).to receive(:new).with(
        host: 'localhost',
        port: 8081,
        endpoint: MsfMcp::Metasploit::JsonRpcClient::DEFAULT_ENDPOINT,
        ssl: true,
        token: 'tok',
        logger: nil
      )
    ...
    it 'includes the invalid api_type in the error message' do
      ...
      }.to raise_error(MsfMcp::Error, 'Invalid API type: grpc')
    end
  ...
  describe 'method delegation' do
    let(:client) do
      allow(MsfMcp::Metasploit::JsonRpcClient).to receive(:new).and_return(jsonrpc_client)
